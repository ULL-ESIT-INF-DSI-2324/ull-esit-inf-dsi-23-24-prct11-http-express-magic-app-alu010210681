...
app.get('/cards', (req, res) => {
  ...
  if (cartaId) {
    GestorCartas.mostrarCarta(usuario as string, parseInt(cartaId.toString()), (error, resultado) => {
      if (error) {
      ...
      GestorCartas.listarCartas(usuario as string, (error, resultado) => {
          if (error) {
...
app.post('/cards', (req, res) => {
  ...
  GestorCartas.addCarta(usuario as string, carta, (error, response) => {
      if (error) {
...
app.patch('/cards', (req, res) => {
  ...
  GestorCartas.modificarCarta(usuario as string, cartaModificada, (error, resultado) => {
      if (error) {
          res.status(400).json({ error: error.message });
      } else {
